...
class TestOptimizedLLM:
    ...
    def test_generate(self, llm):
        ...
        prompt = llm.build_prompt([{"role": "user", "content": "Hello, how are you?"}])
    ...
    def test_stream(self, llm):
        ...
        async def _stream():
            tokens = []
            prompt = llm.build_prompt([{"role": "user", "content": "Hello"}])
            async for token in await llm.stream(prompt):  # noqa: ASYNC110
                tokens.append(token)
            return tokens
